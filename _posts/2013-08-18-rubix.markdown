---
layout: post
title: Of solving the rubix from scratch [Python]
category: posts
published: true
description: Solving the rubix cube from scratch in python.
---

Internet is full of solution for the rubix cube. However, it is seldom described how these solutions were discovered. In this post I'll try to detail how one can solve the rubix cube from scratch.

**Disclaimer : ** The solution presented here is by no mean the fastest... It is actually very long to solve the Rubix Cube using this algorithm. It is just the one I came up with, so I guess it is probably in some sense one of the simplest. Though I am very proud of having cracked it up, there isn't much to be proud about : it took me about a year to come up with a solution. At that time, I was always carrying a rubix cube and a notebook to search for the magic moves in the train like a lunatic.
...
## What is a rubix cube anyway?

### A fixed referential and six possible moves.

I'm pretty sure you know what a rubix cube is.
Let's still make a couple of obvious statements.

We'll assuming that the rubix cube's faces center are
fixed, and that we never rotate the whole thing. Which such
as setting, there is 6 atomic move you can make with a rubix cube. They each consists of turning one of its faces one way or the other.
...
We will associate a 3D-referential to the rubix to 
...
The rubix cube is made of smaller blocks. Let alone the block at the very center of the rubix cube, and the center of the faces,
we have ``3x3x3 - 1 - 6 = 20`` blocks. They are of two types. Corners (8 blocks) and side blocks (12 blocks), showing respectively 3 and 2 faces. While moving the rubix cube, corners will not become side blocks and side blocks will not become corner blocks
...
We will solve the rubix cube the following way. 
...
Representing the rubix cube by a data structure is actually pretty tricky. To keep code small and cute, I chose to avoid implementing a rubix cube class, but instead represent the rubix cube state as a simple dictionary.

In order to solve step 2 or step 3, we will also need
to be able to consider a rubix cube with only side cubes,
a rubix cube with only corner cubes, a rubix cube for which corners are not oriented but are different one to each other (as if they had a number associated) and finally a full rubix cube.

In the following piece of code, the rubix cube will just be a dictionary having for key coordinates in the 3x3x3 3D grid,
and for value an object describe the part of the rubix cube associated (simply called cube in the code).

We use alternatively different implementation of a cube.
One storing orientation information while the other does not.

Note that it would have been possible to use much a much more abstract and efficient way to describe the rubix cube, but I prefer to keep things as explicit as possible here.
...
The code is available [here](/code/rubix/rubix.py),
...
Let's what these operation look like when applied on a resolved rubix cube.

<img src="/images/rubix/initial.jpg">

The operation returned for step 2 is permuting 3 cubes on the back of the rubix cube.

...
def levenshtein(s1, s2, D=2):
    ...
    s1 and s2 is lesser or equal to n
    """
    if D == -1:
        return False
    if len(s1) < len(s2):
    ...
    if len(s2) == 0:
        return len(s1) <= D
    first_letter_cost = 0 if (s1[0] == s2[0]) else 1
    if levenshtein(s1[1:], s2[1:], D-1):
        # deletion
        return True
    if levenshtein(s1, s2[1:], D-1):
        return True
    if s1[0] == s2[0]:    
        # character match
        return levenshtein(s1[1:], s2[1:], D) 
    else:
        # substitution
        return levenshtein(s1[1:], s2[1:], D-1)

{% endhighlight %}

Pretty straightforward, isn't it? This version of the algorithm will unfortunately not help us building our automaton. `s1` and `s2` plays symmetric roles in this code. Ideally we would like to consume the characters of one of the two strings one by one, so let's break this symmetry.

We will munch one character `c` away from s1 at each call. Then we can consider two cases.
Either the character will be dismissed on our path to reach s2, or it will be used. If it is used, it has to be used in a position of at most n in s2.
...
def levenshtein(s1, s2, D=2):
    ...
    for d in range(min(D, len(s2)):
        ...
        # that are required to be added.
        if s1[0] == s2[d]:
            if levenshtein(s1[1:], s2[d+1:], n - d):
                return True
    return False
{% endhighlight %}
...
def levenshtein(s1, s2, D=2):
    ...
    between two string s1 and s2.

    If the distance is greater than D, returns n+1
    """
    def aux(i1, i2, D):
        if i1 == len(s1):
            return len(s2) - i2 <= D
...
def levenshtein(s1, s2, D=2):
    ...
    If the distance is greater than D, returns D+1
    """
    def aux(c, i2, D):
        yield i2, D - 1
        yield i2 + 1, D - 1
...
The next step is to get a DFA from this. This is typically done by running a powerset operation. The cost of the powerset operation is highly dependant on the number of set of states that are accessible. Let's get a reasonable upper bound of that.

To help us figure out what happens, here is a visualization of our Levenshtein Automaton for the word "flees" and a maximum edit distance of 1. You can type in strings (`flyers`, `flee`) in the input below and see what states get activated.

<b>Levenshtein Automaton for flees (type in!)</b>
<div id='levenshtein-simulator'>
</div>

The most striking thing to notice here, is that the set of states that are active at the same time are very close one to each other.
...
At this point, our upper bound for the complexity of the number of set of states in the DFA and its computation time is
    ...
    $$ O \left( (D+1)^{2D + 1}  \right)$$

We also said that our second parameter for each state was the number of error that we can still do and still belong to the language.

So in a sense if `State(n, d)` is active, it does not really matter whether `State(n, d-1)` is active or not, the texts that will match or not in the end will be the same.

# Removing the redundant states

Let's remove the states that are actually imply by other states.

The rule is for any integer d, `(offset, d)` implies `(offset+k, d-abs(k))` as it is just a matter burning our jokers to insert or delete characters.
...
By doing so, we almost completely removed the part that is dependant on the query. We have built an NFA that works on the alphabet of characteristic vectors.
...
Second problem, if we try and apply a powerset transformation blindly on our dfa, we will see that the current number of state is actually infinite. This NFA is actually not finite at all! Imagine it handles queries of any size! Well the trick here is to normalize our states into two parts
...
First the result of the preprocessing is directly serialized, and written directly right into the java code, so that even the cost of the preprocessing is removed.

...
Let's start by having a look at the probability of the sum of two dices... Let's call $X_1$ and $X_2$ the random variable associated with the outcome of the first dice and the second dice respectively.

Then we can compute the probability that their sum is S 

    $$ P(X_1 + X_2 = S) = \sum_{k=1}^{S-1} P(X_1 = k) P(X_2 = S-k) $$
...
We're going to heavily rely on this observation. Given a random variable taking value within integer $0 \leq k \leq n$, we can define a polynomial Q defined as : 

    $$ Q(P) = \sum_{k=0}^{n} P(X=k) X^k $$
...
Finding two dices that solve our problem then boils down to finding pair of polynomials P and Q such that

...
var Application, NFASimulator, React;

React = require('react');

Application = require('./components/LevenshteinApplication.cjsx');

NFASimulator = Application.NFASimulator;

$(function() {
  return React.render(React.createElement(NFASimulator, {
    "query": "flees"
  }), document.getElementById("levenshtein-simulator"));
});


},{"./components/LevenshteinApplication.cjsx":158,"react":157}],2:[function(require,module,exports){
...
var Arrow, Automaton, NFAAutomatonViz, NFASimulator, Node, React, SPACE, Transition;

React = require('react');

NFASimulator = React.createClass({
  getInitialState: function() {
    return {
      word: ""
    };
  ...
  render: function() {
    return React.createElement("div", {
      "className": 'nfa-simulator'
    }, React.createElement("div", {
      "className": 'query'
    }, this.props.query), React.createElement(NFAAutomatonViz, {
...
Transition = (function() {
  function Transition(type, letter1) {
    this.type = type;
    this.letter = letter1 != null ? letter1 : void 0;
  }
...
Automaton = (function() {
  function Automaton(query, n) {
    this.query = query;
    this.n = n != null ? n : 1;
  }

  Automaton.prototype.active_states = function(text) {
    var d, i, j, len, len1, letter, move, new_states, new_states_str, offset, ref, state, state_str, states, transition, v;
    states = {
      "[0,0]": 1
    };
    for (i = 0, len = text.length; i < len; i++) {
      ...
      for (state_str in states) {
        ...
        offset = state[0];
        d = state[1];
        ref = this.transitions(offset, d);
        for (j = 0, len1 = ref.length; j < len1; j++) {
          transition = ref[j];
          if (transition.letter === void 0 || transition.letter === letter) {
            move = {
              h: [1, 0],
              v: [0, 1],
              d: [1, 1],
            ...
            new_states_str = JSON.stringify([offset + move[0], d + move[1]]);
            new_states[new_states_str] = 1;
          }
        }
      }
      states = new_states;
  ...
  Automaton.prototype.transitions = function(offset, d) {
    var i, ref, step, step_types, transitions;
    transitions = [];
    if (d < this.n) {
      transitions.push(new Transition('v'));
      if (offset < this.query.length) {
        transitions.push(new Transition('d'));
      }
    }
    step_types = "hHB";
    for (step = i = 0, ref = this.n - d; 0 <= ref ? i <= ref : i >= ref; step = 0 <= ref ? ++i : --i) {
      if (offset + step < this.query.length) {
        transitions.push(new Transition(step_types[step], this.query[offset + step]));
      }
    }
    return transitions;
  };
...
Node = React.createClass({
  render: function() {
    return React.createElement("g", {
      "className": "node",
      "transform": "translate(" + (SPACE * this.props.x) + "," + (SPACE * (2 - this.props.y)) + ")"
    }, React.createElement("circle", {
      "className": "node-shape " + (this.props.active ? "active" : ""),
      "r": "15"
...
Arrow = React.createClass({
  render: function() {
    var angle, angle_arrow, length, offset, path, ratio, text_off;
    angle = {
      h: 0,
      v: -90,
      d: -45,
    ...
    offset = 10 * ratio;
    path = "M15 0, C" + (15 + offset) + " " + offset + ", " + (length - offset) + " " + offset + ", " + length + " 0";
    return React.createElement("g", {
      "className": "arrow arrow-" + this.props.dir,
      "transform": "translate(" + (SPACE * this.props.x) + "," + (SPACE * (2 - this.props.y)) + ") rotate(" + angle + ")"
    }, React.createElement("path", {
      "d": path
    }), React.createElement("path", {
      "transform": "translate(" + length + ") rotate(" + angle_arrow + ")",
      "className": "marker",
      "d": "M" + 0 + "," + 0 + "L" + -10 + ",-3, L" + -10 + ",3"
    }), React.createElement("text", {
      "transform": "translate(" + (length / 2) + ", " + text_off + ")"
    }, this.props.label));
  }
});

NFAAutomatonViz = React.createClass({
  render: function() {
    var active, active_states, arrows, automaton, d, i, j, k, label, len, nodes, offset, ref, ref1, transition, transitions;
    automaton = new Automaton(this.props.query);
    nodes = [];
    arrows = [];
    active_states = automaton.active_states(this.props.word);
    for (d = i = 0; i <= 1; d = ++i) {
      for (offset = j = 0, ref = this.props.query.length; 0 <= ref ? j <= ref : j >= ref; offset = 0 <= ref ? ++j : --j) {
        active = active_states[JSON.stringify([offset, d])] != null;
        nodes.push(React.createElement(Node, {
          x: offset,
          y: d,
          active: active
        }));
        transitions = automaton.transitions(offset, d);
        for (k = 0, len = transitions.length; k < len; k++) {
          transition = transitions[k];
          label = (ref1 = transition.letter) != null ? ref1 : "*";
          arrows.push(React.createElement(Arrow, {
            x: offset,
            y: d,
            dir: transition.type,
            label: label

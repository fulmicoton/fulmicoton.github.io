...
alphabet = [0] * (MAX_LENGTH_QUERY * NUM_CHAR)
...
class ParametricAutomaton(Automaton):
    ...
    def __init__(self, query, D=2):
        ...
        self.query_letters = set(query)
        self.alphabet = alphabet
        for offset in range(self.len_query + 1):    
            for c in self.query_letters:
                self.alphabet[ord(c) + (offset << 16)] = param_dfa.characteristic(query, c, offset)
    ...
    def step(self, (global_offset, norm_state), c):
        letter = self.alphabet[ord(c) + (global_offset << 16)]
    ...
    def accept(self, (global_offset, final_state_idx)):
        ...
        return (remaining_offset < self.w) and\
            self.accept_values[final_state_idx][remaining_offset]

    def release(self,):
        for offset in range(self.len_query + 1):    
            for c in self.query_letters:
                self.alphabet[ord(c) + (offset << 16)] = 0
